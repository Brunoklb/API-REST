...
const calculate = require('../payment.js/calculator')
...
async function payBankSlip(req, res) {
    ...
    try {
        ...
        if (result) {
            ...
            return res.status(500).json({ message: 'Error updating bankslip status' });
        }
    } catch (err) {
        console.log(err.message);
...
module.exports = { createBankSlip, getBankSlipById, getAllBankSlips, deleteBankSlipById, payBankSlip, cancelBankSlipById };